...
/// Uses ts::PCRAnalyzer from TsDuck.
/// </remarks>
[StructLayout(LayoutKind.Auto)]
...
public sealed class TsDuckMetrics
{
    ...
    public int CalculateQualityScoreEnhanced()
    {
        ...
        if (PcrAnalysis.HasValue)
        {
            ...
            if (pcr.PcrJitterUs > TsDuck.PcrAnalysis.JitterThresholdUs)
            {
                score -= Math.Min(15, (int)((pcr.PcrJitterUs - TsDuck.PcrAnalysis.JitterThresholdUs) / 0.1));
            }

            // PCR interval >100ms violates TR 101 290
            if (pcr.PcrIntervalMs > TsDuck.PcrAnalysis.IntervalThresholdMs)
            {
                score -= Math.Min(10, (int)((pcr.PcrIntervalMs - TsDuck.PcrAnalysis.IntervalThresholdMs) / 10));
            }
        ...
        if (IatAnalysis.HasValue)
        {
            ...
            if (iat.IatJitterUs > TsDuck.IatAnalysis.JitterThresholdUs)
            {
                score -= Math.Min(10, (int)((iat.IatJitterUs - TsDuck.IatAnalysis.JitterThresholdUs) / 500));
            }
            ...
            if (iat.BurstCount > TsDuck.IatAnalysis.BurstThreshold)
            {
                score -= Math.Min(5, (int)(iat.BurstCount - TsDuck.IatAnalysis.BurstThreshold));
            }
        }

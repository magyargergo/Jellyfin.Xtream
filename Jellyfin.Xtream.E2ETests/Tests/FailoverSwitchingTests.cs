...
public class FailoverSwitchingTests(DockerTestFixture fixture, ITestOutputHelper output)
    : NativeE2ETestBase(output, TestConfigs.AggressiveFailover)
{
    /// <summary>
    ...
    public async Task ManualSwitchDuringReconnection_RotatesToNextUrl()
    {
        ...
        var stableUrl2 = $"{fixture.BaseUrl}/stream/5000";
        var stableUrl3 = $"{fixture.BaseUrl}/stream/5000";
        ...
        // Assert
        Output.WriteLine($"After: State={statusAfter.State}, URL={statusAfter.CurrentUrlIndex}");
        Output.WriteLine($"Bytes: {statusAfter.BytesReceived:N0}");
        Output.WriteLine($"Switches: {statusAfter.SwitchesCompleted}");
        Output.WriteLine($"Reconnections: {statusAfter.Reconnections}");
    ...
    public async Task AutoFailoverAfterManualSwitch_ChainsCorrectly()
    {
        ...
        var stableUrl1 = $"{fixture.BaseUrl}/stream/5000";
        ...
        // Assert
        Output.WriteLine($"Final: State={statusAfter.State}, URL={statusAfter.CurrentUrlIndex}");
        Output.WriteLine($"Bytes: {statusAfter.BytesReceived:N0}");
        Output.WriteLine($"Switches: {statusAfter.SwitchesCompleted}");
        Output.WriteLine($"Reconnections: {statusAfter.Reconnections}");
    ...
    public async Task UrlRotation_WrapsToFirstUrl()
    {
        ...
        // Assert
        Output.WriteLine($"Final: State={finalStatus.State}, URL={finalStatus.CurrentUrlIndex}");
        Output.WriteLine($"Switches: {finalStatus.SwitchesCompleted}");
        Output.WriteLine($"URL index history: [{string.Join(", ", urlIndexHistory)}]");
        ...
        Output.WriteLine($"URL visit counts: 0={countZero}, 1={countOne}, 2={countTwo}");
    ...
    public async Task MetricsAcrossMultipleFailovers_RemainValid()
    {
        ...
        using var streamer = BuildStreamer(TestConfigs.AnalyzerFailover);
        ...
        if (metrics != null)
        {
            ...
            Output.WriteLine($"PMT errors: {metrics.Priority1.PmtError}");
    ...
    public async Task RapidSwitchDuringFailover_HandledGracefully()
    {
        ...
        fixture.UnstableDropAfterMs = 1000;
        var unstableUrl = $"{fixture.BaseUrl}/stream/unstable";
    ...
    public async Task SwitchCounters_AccurateDuringMixedOperations()
    {
        ...
        // Assert
        Output.WriteLine($"Final: Switches={finalStatus.SwitchesCompleted}, Reconnections={finalStatus.Reconnections}");
        Output.WriteLine($"Final URL: {finalStatus.CurrentUrlIndex}");
    ...
    public async Task KeyframeAlignment_AfterManualSwitch_StreamQualityRemains()
    {
        ...
        Output.WriteLine($"Sync errors: {metricsAfterSwitch.Priority1.SyncByteError}");
        Output.WriteLine($"Switches: {status.SwitchesCompleted}");
    ...
    public async Task KeyframeAlignment_EmitsKeyframeFoundEvent()
    {
        ...
        // Assert
        Output.WriteLine($"Initial URL: {initialUrlIndex}, Final URL: {status.CurrentUrlIndex}");
        Output.WriteLine($"Switch detected: {switched}");
        Output.WriteLine($"Switches completed: {status.SwitchesCompleted}");

...
public class DnsFailoverTests
{
    private readonly DockerTestFixture _fixture;
    private readonly ITestOutputHelper _output;

    public DnsFailoverTests(DockerTestFixture fixture, ITestOutputHelper output)
    {
        _fixture = fixture;
        _output = output;
    }
    ...
    public void DnsFailure_TracksFailureCount_Correctly()
    {
        ...
        _fixture.ConfigureProvider(provider1, new ProviderBehavior { BitrateKbps = 5000 });
        ...
        // Assert
        _output.WriteLine($"Provider 0 initial count: {count0Initial}");
        _output.WriteLine($"Provider 0 after 1 failure: {count0After1}");
        _output.WriteLine($"Provider 0 after 2 failures: {count0After2}");
        _output.WriteLine($"Provider 1 initial count: {count1Initial}");
        _output.WriteLine($"Provider 1 after 1 failure: {count1After1}");
    ...
    public void DnsFailure_BelowThreshold_ReturnsSwitchPolicy()
    {
        ...
        _fixture.ConfigureProvider(provider, new ProviderBehavior { BitrateKbps = 5000 });
        ...
        streamer.AddUrl(_fixture.GetProviderUrl(provider));
        ...
        // Assert
        _output.WriteLine($"After 1 failure: policy={policy1}, count={count1}");
        _output.WriteLine($"After 2 failures: policy={policy2}, count={count2}");
    ...
    public void DnsFailure_ReachesThreshold_ReturnsEjectPolicy()
    {
        ...
        // Assert
        _output.WriteLine($"Policy after 1 failure: {policy1}");
        _output.WriteLine($"Policy after 2 failures: {policy2}");
        _output.WriteLine($"Policy after 3 failures: {policy3}");
        _output.WriteLine($"Provider state after ejection: {state}");
        _output.WriteLine($"DNS failure count after ejection: {countAfterEjection}");
    ...
    public void DnsFailure_SuccessResetsCount()
    {
        ...
        // Assert
        _output.WriteLine($"Count before reset: {countBeforeReset}");
        _output.WriteLine($"Count after reset: {countAfterReset}");
    ...
    public void DnsFailure_TrackedIndependentlyPerProvider()
    {
        ...
        // Assert
        _output.WriteLine($"Provider 0: state={state0}, count={count0}, policy3={policy3}");
        _output.WriteLine($"Provider 1: state={state1}, count={count1}");
    ...
    public async Task DnsFailure_DuringStreaming_SwitchesToHealthyProvider()
    {
        ...
        _fixture.ConfigureProvider(primaryProvider, new ProviderBehavior { BitrateKbps = 5000 });
        _fixture.ConfigureProvider(backupProvider, new ProviderBehavior { BitrateKbps = 5000 });
        ...
        streamer.AddUrl(_fixture.GetProviderUrl(primaryProvider));
        streamer.AddUrl(_fixture.GetProviderUrl(backupProvider));
        ...
        // Assert
        _output.WriteLine($"Primary provider state: {primaryState}");
        _output.WriteLine($"Backup provider state: {backupState}");
        _output.WriteLine($"Current URL index: {status.CurrentUrlIndex}");
        _output.WriteLine($"Bytes received: {status.BytesReceived:N0}");
        _output.WriteLine($"Policy 3 (should be EjectAndSwitch): {policy3}");
    ...
    public async Task DnsFailure_AfterQuarantine_ProviderCanRecover()
    {
        ...
        using var streamer = CreateStreamerWithShortQuarantine();
        if (streamer == null)
        {
            _output.WriteLine("SKIP: Native library not available");
            return;
        ...
        _output.WriteLine($"State after ejection: {stateAfterEject}");
    ...
    private static NativeStreamer? CreateStreamerWithShortQuarantine()
    {
        // Use highly reactive config with short quarantine

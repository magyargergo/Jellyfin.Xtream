...
// ============================================================================
// V2 API: Streamer with SQLite metrics and shared memory
// ============================================================================

namespace {

/// Helper to build StreamerConfig from native config
streaming::StreamerConfig buildStreamerConfig(const TsDuckStreamerConfigNative* config) {
    streaming::StreamerConfig cfg{};
    if (config != nullptr) {
        cfg.connect_timeout_ms = config->connect_timeout_ms;
        cfg.response_timeout_ms = config->response_timeout_ms;
...
{
    LOG_DEBUG(kStreamer, "tsduck_streamer_create_v2 called db=%s shm=%s size=%lld",
              metrics_db_path ? metrics_db_path : "(null)",
              shared_buffer_name ? shared_buffer_name : "(null)",
              static_cast<long long>(shared_buffer_size));

    try {
        streaming::StreamerConfig cfg = buildStreamerConfig(config);

        std::string db_path = metrics_db_path ? metrics_db_path : "";
        std::string shm_name = shared_buffer_name ? shared_buffer_name : "";
        size_t shm_size = shared_buffer_size > 0 ? static_cast<size_t>(shared_buffer_size) : 0;

        // NOLINTBEGIN(cppcoreguidelines-owning-memory)
        auto* pipeline = new streaming::StreamPipeline(cfg, analyzer_config, db_path, shm_name, shm_size);
        LOG_DEBUG(kStreamer, "tsduck_streamer_create_v2 done, handle=%p", static_cast<void*>(pipeline));
        return toHandle(pipeline);
        // NOLINTEND(cppcoreguidelines-owning-memory)
    } catch (const std::exception& e) {
        LOG_ERROR(kStreamer, "tsduck_streamer_create_v2 exception: %s", e.what());
        return nullptr;
    } catch (...) {
        LOG_ERROR(kStreamer, "tsduck_streamer_create_v2 unknown exception");
        return nullptr;
    }
}

// ============================================================================
// V2 API: Metrics Database Queries
// ============================================================================
    ...
    TsDuckProviderHealthNative* out_health)
{
    ...
    auto* db = impl->metrics_db();
    if (db == nullptr) {
        return TSDUCK_ERROR_NOT_INITIALIZED;
    }

    metrics::ProviderHealth health{};
    if (!db->get_provider_health(url_index, &health)) {
        return TSDUCK_ERROR_INVALID_DATA;
    }

    // Copy to native structure
    out_health->url_index = health.url_index;
    out_health->health_score = health.health_score;
    out_health->total_bytes = health.total_bytes;
    out_health->success_count = health.success_count;
    out_health->failure_count = health.failure_count;
    out_health->avg_latency_ms = health.avg_latency_ms;
    out_health->last_success_time = health.last_success_time;
    out_health->last_failure_time = health.last_failure_time;
    out_health->consecutive_failures = health.consecutive_failures;
    ...
    int32_t max_count)
{
    ...
    if (out_health_array == nullptr || max_count <= 0) {
        return TSDUCK_ERROR_INVALID_DATA;
    }

    auto* db = impl->metrics_db();
    if (db == nullptr) {
        return 0;  // No database, no providers
    }

    auto all_health = db->get_all_provider_health();
    int32_t count = std::min(static_cast<int32_t>(all_health.size()), max_count);

    for (int32_t i = 0; i < count; ++i) {
        const auto& h = all_health[i];
        out_health_array[i].url_index = h.url_index;
        out_health_array[i].health_score = h.health_score;
        out_health_array[i].total_bytes = h.total_bytes;
        out_health_array[i].success_count = h.success_count;
        out_health_array[i].failure_count = h.failure_count;
        out_health_array[i].avg_latency_ms = h.avg_latency_ms;
        out_health_array[i].last_success_time = h.last_success_time;
        out_health_array[i].last_failure_time = h.last_failure_time;
        out_health_array[i].consecutive_failures = h.consecutive_failures;
        out_health_array[i].reserved = 0;
    }
...
TSDUCK_API int32_t tsduck_streamer_get_provider_count(TsDuckStreamerHandle streamer)
{
    ...
    auto* db = impl->metrics_db();
    return db ? db->get_provider_count() : 0;
}

TSDUCK_API void tsduck_streamer_reset_all_health(TsDuckStreamerHandle streamer)
{
    auto* impl = toImpl(streamer);
    if (impl == nullptr) {
        return;
    ...
    int64_t max_age_seconds)
{
    ...
    auto* db = impl->metrics_db();
    if (db != nullptr) {
        db->cleanup_old_records(max_age_seconds);
    }
}

#else  // !TSDUCK_HAS_SQLITE - Stub implementations

TSDUCK_API int32_t tsduck_streamer_get_provider_health(
    TsDuckStreamerHandle,
    int32_t,
...
TSDUCK_API int32_t tsduck_streamer_get_provider_count(TsDuckStreamerHandle)
{
    return 0;
}

TSDUCK_API void tsduck_streamer_reset_all_health(TsDuckStreamerHandle)
{
}

TSDUCK_API void tsduck_streamer_cleanup_metrics(TsDuckStreamerHandle, int64_t)
{
}
...
{
    auto* impl = toImpl(streamer);
    if (impl == nullptr || out_path == nullptr || max_length <= 0) {
        return 0;
    }

    const auto& path = impl->metrics_db_path();
    if (path.empty()) {
        out_path[0] = '\0';
        return 0;
    }

    int32_t len = static_cast<int32_t>(path.length());
    int32_t copy_len = std::min(len, max_length - 1);
    std::memcpy(out_path, path.c_str(), copy_len);
    out_path[copy_len] = '\0';
    ...
    TsDuckSharedBufferInfo* out_info)
{
    ...
    auto* buf = impl->shared_buffer();
    if (buf == nullptr || !buf->is_valid()) {
        return false;
    }

    out_info->name_length = static_cast<int32_t>(buf->get_name().length());
    out_info->total_size = static_cast<int64_t>(buf->get_total_size());
    out_info->buffer_size = static_cast<int64_t>(buf->get_buffer_size());
    out_info->header_offset = 0;
    out_info->data_offset = sizeof(streaming::SharedBufferHeader);
...
{
    auto* impl = toImpl(streamer);
    if (impl == nullptr || out_name == nullptr || max_length <= 0) {
        return 0;
    }

    const auto& name = impl->shared_buffer_name();
    if (name.empty()) {
        out_name[0] = '\0';
        return 0;
    }
    ...
    int32_t copy_len = std::min(len, max_length - 1);
    std::memcpy(out_name, name.c_str(), copy_len);
    out_name[copy_len] = '\0';

    return len;
}

TSDUCK_API int64_t tsduck_streamer_get_shared_buffer_size(TsDuckStreamerHandle streamer)
{
    auto* impl = toImpl(streamer);
    if (impl == nullptr) {
        return 0;
    ...
    auto* buf = impl->shared_buffer();
    return buf ? static_cast<int64_t>(buf->get_total_size()) : 0;
}

TSDUCK_API void tsduck_streamer_reset_shared_buffer(TsDuckStreamerHandle streamer)
{
    auto* impl = toImpl(streamer);
    if (impl != nullptr) {
        impl->reset_shared_buffer();
    }
}
